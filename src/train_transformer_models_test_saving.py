...
import argparse
from utils.model_utils import CustomModel
from utils.train_transformer import train_and_evaluate_transformer, prepare_data, evaluate_transformer_test_set
...
def main(out_path, test_size, val_size, n_sweeps, n_epochs):
    ...
    evaluate_transformer_test_set(sweep_project_name, 
                              transformer_model, 
                              train_dataloader_tri, 
                              eval_dataloader_tri, 
                              test_dataloader_tri,
                              test_dataloader_auto, 
                              out_path, 
                              "TEST_triangle_transformer_confusion_matrix.png"
                              "TEST_triangle_transformer_classification_report.txt", 
                              1)
    

    # Try to load model
    model=CustomModel(checkpoint='chcaa/dfm-encoder-large-v1',
                                    num_labels=2).to(device)

    model.load_state_dict(torch.load("../output/transformers/best_models/model.pt", map_location=device))
